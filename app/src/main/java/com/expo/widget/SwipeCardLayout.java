...
//        mDy = getContext().getResources().getDimension(R.dimen.dms_50);
//        mDy = density * 5;
        paint = new Paint();
        ...
        round = getContext().getResources().getDimension(R.dimen.dms_10);/*density * 8;*/
    }
    ...
    public void setAdapter(CardAdapter adapter) {
        ...
        bindLayout1.setElevation(4F);
        swipeLayout1.addView(bindLayout1);

        addView(swipeLayout1);

        Object data = adapter.tQueue.poll();
        adapter.bindData(data, bindLayout1);
        adapter.tQueue.add(data);
        data = adapter.tQueue.poll();
        adapter.bindData(data, bindLayout);
        adapter.tQueue.add(data);
    }
        ...
        public boolean onTouchEvent(MotionEvent event) {
            ...
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (!isMove && Math.abs(dx) < slop && Math.abs(dy) < slop) {
                        return false;
                    }
                    isMove = true;
                ...
                case MotionEvent.ACTION_UP:
                    if (isMove) {
                            ...
                            animator.addListener(new Animator.AnimatorListener() {
                                ...
                                public void onAnimationEnd(Animator animation) {
                                    ...
                                    if (adapter.tQueue.size() > 0) {
                                        ...
                                        adapter.bindData(adapter.tQueue.element(), childAt.getChildAt(0));
                                        adapter.tQueue.add(adapter.tQueue.poll());
                                        position = position + 1 >= 3 ? 0 : position + 1;
                                    } else {
                        ...
                        return ((ViewGroup) getParent()).callOnClick();
                    }
            }
            return false;
        }
