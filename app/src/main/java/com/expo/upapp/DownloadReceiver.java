...
//            installApk(context, id, PublicUtile.FILE_NAME);
            if (id == UpdateAppManager.downloadId)
                LocalBroadcastUtil.sendBroadcast(context, null, Constants.Action.ACTION_DOWNLOAD_APP_SUCCESS);
            else if (id == UpdateAppManager.vrAppDownloadId)
                LocalBroadcastUtil.sendBroadcast(context, null, Constants.Action.ACTION_DOWNLOAD_VR_APP_SUCCESS);
